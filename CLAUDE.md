...
- **Packs × cidade: landing a sério desde 2026-09-23, e as regalias são as do quiz.** As 244 páginas `/{pack}-{cidade}` (4 packs × 61 cidades) deixaram de ser o configurador nu de `/packs` e passaram a ter página própria, `src/pages/PackComboPage.tsx`: hero comercial (`CommercialHero`, migalha + CTA de WhatsApp + antes/depois + estatísticas), bloco de regalias, configurador, avaliações, guia do processo, FAQ com schema e ligações internas. **`/packs` (`CustomPackPage.tsx`) não mudou de layout** — o dono está a retrabalhá-la; as duas páginas partilham `src/components/PackConfigurator.tsx`, que é a única cópia do configurador (mesma regra do `PriceWidget`).
  - **A regalia do pack é preço por artigo, não desconto sobre o pedido.** O primeiro artigo paga tabela; cada artigo acrescentado entra com preço de pack: sofá 1L 35€ / 2L 55€ / 3L 65€, colchão menos 14€, uma cadeira oferecida por cada 4. São exatamente os números do ecrã de upsell do quiz, e desde esta data vivem em **`src/constants/packPerks.ts`**, importados pelos dois lados — o `QuizComboUpsellScreen.tsx` deixou de os ter escritos à mão. **A regalia é sobre a limpeza**: um artigo acrescentado com impermeabilização, anti-ácaros ou desbacterização mantém o preço de tabela, tal como no quiz.
