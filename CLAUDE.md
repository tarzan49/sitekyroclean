...
- **Taxa de deslocação:** mínimo **10€ em todo o site** (nenhuma cidade a 5€). A única exceção existe no código e é de propósito: `calculateTravelFee` (`travel.ts`) dispensa a deslocação (0€) quando os serviços passam de 120€ nas cidades a 10€, de 135€ nas a 15€ e de 150€ nas a 20€. Fonte única: `src/constants/travel.ts` (`locationPrices`) — `QuizTypes.ts` e `locationSeoData.ts` só reexportam/importam, já não há cópias paralelas (armadilha resolvida). **Aveiro e Coimbra: 15€** (adicionadas como cidades reais em 2026-09-10, servidas pela equipa Porto por não haver equipa própria no Centro).
- **Oito cidades novas a 2026-09-26, porque as campanhas de Google Ads já as abrangiam e o questionário respondia "Localidade não encontrada".** Sul do Douro (equipa Porto): **Santa Maria da Feira 10€** (~27 min, como Espinho), **São João da Madeira, Ovar e Oliveira de Azeméis 15€** (~33-37 min, como Penafiel). Alentejo Litoral (equipa Lisboa): **Alcácer do Sal, Grândola, Santiago do Cacém e Sines 15€**, o teto de Lisboa, pelo mesmo critério que o dono aplicou a Coimbra (rejeitou 20€/25€ para as cidades afastadas). **Os valores foram deduzidos das zonas existentes, não ditos pelo dono**: se ele os mudar, muda-se só o `travel.ts`. `travelPrices.test.ts` passou a exigir que `locationPrices` e `cities` tenham exatamente as mesmas cidades — **uma cidade que entre na segmentação dos anúncios tem de entrar nos dois**, senão o anúncio leva a um questionário que a recusa.
- **"Disponibilidade sob consulta" vem de `EXTENDED_TRIP_CITIES`** (`travel.ts`): Aveiro, Coimbra e as quatro do Alentejo Litoral. Estava escrito à mão como `Aveiro || Coimbra` em três sítios.
- **A pesquisa do passo de localidade encontra freguesias e localidades** (`searchServiceLocations` em `locationDetection.ts`): "Comporta" mostra "Comporta, Alcácer do Sal" e escolhê-la seleciona o concelho, que é o que tem taxa. Também expande "S." / "Sta." / "Sto.". Tróia, Furadouro e Vila Nova de Santo André entram como entradas de freguesia pelo mesmo critério de Vilamoura em Loulé. **Um nome de freguesia que já exista noutro concelho servido não se acrescenta** (Alvalade, Carvalhal): a deteção por GPS resolve freguesias pelo nome e trocaria o concelho.
...
- **Tapetes nunca mostram preço, em lado nenhum (desde 2026-09-06).** O quiz interativo deixou de estimar preço por m² — é sempre "Sob Orçamento". O passo "Detalhes do(s) Tapete(s)" (`QuizStepConfigCarpet.tsx`) virou um simulador: a pessoa mede cada tapete (largura × comprimento, `CarpetItem[]` em `QuizTypes.ts`) e pode adicionar quantos quiser via "+ Adicionar outro tapete", sem limite. O antigo motor de preço por escalão (`calcCarpetPrice`/`CARPET_TIERS`/`carpetActiveTier`) foi removido de `quizHelpers.ts` — já não existe em lado nenhum do quiz. O widget de preços de marketing (`PriceWidget.tsx`, ver nota abaixo) também nunca mostra preço de tapete, e desde 2026-09-09 a alcatifa é igualmente sob orçamento (o antigo `calcCarpetWidget` a 3€/m² foi removido).
- **Cadeiras: decisão de addon movida para depois do "Continuar" (desde 2026-09-06).** Quando a limpeza é o serviço principal, o passo "Detalhes das Cadeiras" só pergunta quantidade — a decisão de impermeabilização/anti-ácaros passou para um ecrã novo, estilo "upsell de companhia aérea" (`QuizChairsAddonUpsell.tsx`), que aparece uma única vez logo a seguir ao "Continuar" dessa etapa,. Nesse ecrã: Impermeabilização continua Premium/Essencial (Premium com selo "TOP", Essencial sempre disponível, nunca remover uma das duas sem pedido explícito — já aconteceu por engano); Anti-ácaros para cadeiras é uma opção mutuamente exclusiva com a impermeabilização, preço sempre **5€ por cadeira, fixo, mostrado só como taxa unitária "5€/un." nunca como total calculado** nos ecrãs de escolha (pedido explícito, repetido duas vezes; o resumo do pedido mostra o valor da linha para as linhas somarem). Esteve retirada de 2026-09-10 a 2026-09-26 (o ecrã dizia que vinha "incluída na impermeabilização"); voltou por decisão do dono, ver "Anti-ácaros" abaixo. A fórmula escalonada do widget (`calcChairAntiAcarosTotal`, 10€ + 7,50€/cadeira) era código morto com um terceiro preço e foi apagada.
- **Packs × cidade: landing a sério desde 2026-09-23, e as regalias são as do quiz.** As 244 páginas `/{pack}-{cidade}` (4 packs × 61 cidades) deixaram de ser o configurador nu da antiga `/packs` e passaram a ter página própria, `src/pages/PackComboPage.tsx`: hero comercial (`CommercialHero`, migalha + CTA de WhatsApp + antes/depois + estatísticas), bloco de regalias, configurador, avaliações, guia do processo, FAQ com schema e ligações internas. **`/packs` (`CustomPackPage.tsx`) foi removida a 2026-09-24** e responde 301 para `/guia-de-packs` (`public/_redirects`); o `CustomPackPage.tsx` já não existe. `src/components/PackConfigurator.tsx` é a única cópia do configurador (mesma regra do `PriceWidget`) e vive nas páginas de pack.
  - **A regalia do pack é preço por artigo, não desconto sobre o pedido.** O artigo principal paga tabela **em todas as unidades e tamanhos**; cada artigo de **outro tipo** acrescentado à mesma visita entra com preço de pack, só a partir de 100€ de subtotal de tabela (`PACK_PERK_MIN_ORDER`): sofá 1L 35€ / 2L 55€ / 3L 65€, colchão menos 14€, uma cadeira oferecida por cada 4. São exatamente os números do ecrã de upsell do quiz, e desde esta data vivem em **`src/constants/packPerks.ts`**, importados pelos dois lados — o `QuizComboUpsellScreen.tsx` deixou de os ter escritos à mão. **Desde 2026-09-26 a própria regra é uma só função, `priceWithPackPerks()` em `packPerks.ts`, usada pelo quiz e pelo configurador.** Antes o configurador dava preço de pack a qualquer linha depois da primeira (sofá 3L + 2L custava 134€ no configurador e 148€ no quiz) — o dono: "o site tem de ter consistência nos preços, senão parece um negócio duvidoso". `src/lib/packPricingParity.test.tsx` passa os mesmos cestos pelos dois lados e rebenta se os totais divergirem. A regalia do tapete ("Limpe 5 m², pague 4", `PACK_PERK_RUG_NOTE`) também vive lá e também só vale a partir dos 100€. **A regalia é sobre a limpeza**: um artigo acrescentado com impermeabilização, anti-ácaros ou desbacterização mantém o preço de tabela, tal como no quiz.
  - **O desconto de 10% acima de 149€ não existe e não se volta a anunciar.** Foi retirado do código a 2026-09-10 e continuou escrito no HTML estático das 244 páginas de pack e no `/packs` até 2026-09-23: uma promessa que só os motores liam, exatamente o que a regra anti-cloaking proíbe. `src/constants/packPerks.test.ts` rebenta se uma página de pack (React ou prerender) voltar a falar em percentagem de desconto ou no limiar dos 149€, e se o quiz ou o configurador reescreverem os preços à mão.
  - **O prerender dos packs passou a dizer o que a página mostra:** migalha, regalias, o que a visita inclui, 4 FAQs por pack × cidade e ligações para os dois serviços dessa cidade, para os outros packs e para o guia de packs. O `<main>` foi de ~1.500 para ~7.000 caracteres e ganhou `Service` + `BreadcrumbList` + `FAQPage` (era a única família sem nenhum JSON-LD). Conteúdo novo escreve-se em `packIncludes`/`packFaqs` (`src/data/packComboData.ts`), que os dois lados leem.
  - **Nomes dos packs = o que se pesquisa (2026-09-24, dono).** Os títulos são "Limpeza de Sofá e Colchão", "Limpeza e Impermeabilização de Sofá", "Limpeza de Sofá, Cadeiras e Tapete" e "Limpeza de Colchão e Tapete": "pack" quase não tem procura, e **sem "+"** no título. Os slugs `pack-*` ficaram iguais. Rodapé e prerender leem os nomes de `packComboData.ts`, não os escrevem à mão.
  - **Configurador: sem desbacterização, tratamento em 3 escolhas (2026-09-24, dono).** Desbacterização e anti-ácaros são o mesmo tratamento e vendiam-se como dois; no configurador só existe "Anti-ácaros". O tratamento é uma lista: Só limpeza / Impermeabilizar / Anti-ácaros (colchão: só limpeza ou anti-ácaros). Premium e Essencial só aparecem depois de escolher Impermeabilizar. Selo TOP no Anti-ácaros e no Premium. As páginas `/desbacterizacao` do resto do site não foram tocadas.
  - **Bug resolvido no caminho:** o seletor de localidade do configurador listava Aveiro e Coimbra duas vezes, desde que passaram a cidades reais em `serviceCatalog`.

- **Upsell final "Aproveite a mesma visita" (desde 2026-09-06; o título "poupe 10%" e o desconto de 10% já não existem).** O antigo `QuizUpsellOverlay.tsx` (escolher um item de cada vez) foi completamente removido e substituído por `QuizComboUpsellScreen.tsx`: um único ecrã com as 3 categorias (Colchão, Sofá, Cadeiras) lado a lado, cada uma abre a sua própria página de quantidades com tamanhos/preços reais (não uma quantidade genérica). Continua a escrever no mesmo array `upsellItems`/`setUpsellItems` que já existia. Os preços de cada artigo são os de pack (`priceWithPackPerks`). **Os cartões do resumo pré-visualizam o preço como se o artigo de referência já estivesse no pedido** (bug visto em produção a 2026-09-26: com um pedido principal abaixo de 100€, o cartão do colchão mostrava "~~69€~~ 69€"), e o `PriceCompare` não risca quando os dois preços são iguais.
- **Anti-ácaros vende-se em sofás, colchões e cadeiras (dono, 2026-09-26), no quiz e no configurador, aos mesmos preços.** Fonte única: `src/constants/antiAcarosPricing.ts` (sofá por tamanho, cadeira 5€/un., colchão = `bothPrice − cleaningPrice`), lido pelo quiz (`QuizSofaAddonUpsell`, `QuizChairsAddonUpsell`, `QuizMattressAddonUpsell`), pelo `use-quiz-pricing`, pelo recibo do `submissionService` e pelo `customPack`. **Um tratamento por artigo:** só limpeza, impermeabilização (Premium ou Essencial) ou anti-ácaros, nunca dois. Desbacterização é o mesmo tratamento, com outro nome. Antes disto, o quiz só o vendia no colchão e o configurador vendia-o nos três.
- **Chaise longue: não é uma opção nem tem preço próprio (dono, 2026-09-26: "não acrescentar, remover").** Estava anunciada a +10€ nas tabelas de preço de ~16k páginas, no blog e no configurador, e o quiz nunca a perguntava nem cobrava. Foram removidos o `sofaChaisePrice`, o campo `sofaHasChaise`, o botão do configurador, as linhas do `PRICE_TABLE` e as frases de preço. Menções descritivas (um sofá com chaise num guia, as avaliações de clientes) ficam. Não reintroduzir sem pedido explícito.
- **O pack limpeza + impermeabilização Essencial do sofá começa em 89€ (dono, 2026-09-26)** — é o valor do motor (`calcPackPricing`: `bothPrice − waterproofingUpsellDiscount`). O hero de `/impermeabilizacao*` e as páginas de problema diziam 99€ (o `bothPrice` cru) e passaram a ler o motor. Os dois anúncios de impermeabilização do Google Ads diziam "desde 99€" e passaram a "desde 89€" no mesmo dia (título e descrição, conta e `docs/google-ads/`); o `gerar-csv.py` lê agora o preço de pack do motor, não o `bothPrice` cru.
- **O `PRICE_TABLE` (`locationPriceTestimonialsData.ts`) deriva do motor desde 2026-09-26** (`sofaPrices`, `mattressPrices`, `calcChairClean`), em vez de texto escrito à mão que o widget voltava a converter em números.
...
Segunda instância do mesmo problema: `src/pages/admin/SitemapMonitor.tsx` (era o `AdminPanel.tsx`) mantém o seu próprio array `SITEMAPS` e switch `getSitemapUrls()`, escritos à mão — novos sub-sitemaps adicionados a `scripts/generate-sitemap.ts` **não aparecem automaticamente** ali, é preciso atualizar os dois.
...
- **Fase 8 (consolidação das páginas-pilar e off-site), começada em 2026-09-18.** As **páginas-pilar** nunca tinham sido definidas em lado nenhum: são os seis serviços (`/limpeza-sofas`, `/limpeza-colchoes`, `/limpeza-tapetes`, `/limpeza-cadeiras`, `/limpeza-alcatifas`, `/impermeabilizacao`), porque as 16.000 páginas geradas são todas combinações de um deles com uma cidade, freguesia, material, preço, marca ou problema. O `references/pyramid-structure.md` **não** descreve este site: é o spec genérico de outro toolkit.
  ...
  - **Por fazer:** as 24 páginas EN continuam sem hub (não existe `/en`) e o dono ainda não confirmou se o turismo o justifica; `/guia-de-packs` deixou de ser órfã (é o passo "Packs" da migalha das 244 páginas de pack, e o destino do 301 de `/packs`, que já não existe).
...
- **Auditoria de 2026-09-23 (corrigido e publicado).** Quatro coisas que só se viam no site construído, não no código:
  ...
  - **`lastmod` em produção: resolvido.** Faltava porque o Cloudflare clona shallow (produção chegou a ter `lastmod` em 26 URLs). O `scripts/unshallow.mjs` corre como `prebuild` e aprofunda o clone; a 2026-09-26 produção tinha `lastmod` nos 17.214 URLs. **Nunca hardcodar datas:** se voltar a faltar, a causa é o clone.
  ...
  - **Segunda ronda, sobre o JSON-LD do site construído (corrigido e publicado).** Nada disto se vê a ler o código:
  ...
  - **CSP: a bloquear desde 2026-09-23, e como se lá chegou.** A política vive em `public/_headers` (comentada em detalhe lá). Três coisas para não repetir:
    ...
    - **`connect-src` tinha o mesmo problema do `img-src`, só descoberto em 2026-09-24 a submeter um pedido a sério em produção.** A etiqueta de conversão do Ads (`AW-18457115875/ewA0CLXn_fscEOP5hOFE`) está instalada e dispara corretamente no `dataLayer` (com `lead_id` e valor certos), mas o `gtag` envia essa conversão por `fetch`/`sendBeacon` para o domínio Google do PAÍS de quem visita (`www.google.dk`, visto num teste real), não para `www.google.com`. Como `connect-src` só tinha `https://www.google.com`, o pedido era bloqueado e a conversão nunca chegava ao Ads — silenciosamente. Corrigido com a mesma solução do `img-src`: `https:` aceite em `connect-src`, documentado no próprio `public/_headers`. **A etiqueta de conversão do Ads já não está em falta** (contradiz a nota mais antiga desta secção); o que estava em falta era isto.
...
- **`#D4AF37` é o único dourado da Kyro, em qualquer fundo — ícones, badges, bordas de destaque, estrelas.** Corrigido em 2026-09-24: a nota anterior aqui ("`#D4AF37` só em fundo escuro", com `#8B6914`/`#aa862b` como substitutos em fundo claro) estava errada e nunca foi uma regra do dono — era uma dedução minha a partir de contraste de texto, generalizada por engano a ícones e a decoração. O dono confirmou explicitamente: quer o mesmo dourado do fundo escuro também no branco. **A única exceção real é texto corrido pequeno em fundo claro**, onde `#D4AF37` falha o contraste (2,1:1) — aí, e só aí, usar `#8B6914` (texto pequeno) ou `#aa862b` (palavra destacada de título). Um ícone, badge ou borda nunca cai nessa exceção. Antes de aplicar um dourado a um elemento novo, checar se é texto corrido pequeno (exceção) ou tudo o resto (`#D4AF37`) — não copiar um hex de outro componente sem saber a qual dos dois casos ele pertence. **O `#B8912A`, `#8D7020` e outras variantes espalhadas pelo código são divergência antiga, não uma segunda regra válida** — ao mexer num componente que os usa, alinhar com esta regra em vez de propagar.
...
- **"Tapete: Limpe 5 m², pague 4" no upsell final é intencional** (dono, 2026-09-23): é um desconto sobre o orçamento que vier a ser feito, não um preço por m². Não remover. Desde 2026-09-26 vale nas mesmas condições das outras regalias (a partir de 100€ de subtotal) e aparece também no configurador de packs.
...
- **Por fazer, do lado do Cloudflare:** substituir `functions/_middleware.ts` por uma Redirect Rule (ver AUDIT.md, ponto 3). E criar o registo DNS `www`: a 2026-09-26 `www.cleansolutions.com.pt` não resolvia, por isso quem escreve o endereço com `www` recebe um erro do browser em vez do site. **Por esta ordem: primeiro a Redirect Rule `www` → `https://cleansolutions.com.pt` (301, mantendo o caminho), só depois o registo DNS.** Não acrescentar `www` como domínio próprio do Pages (ver "Correções de 2026-09-26").

## Correções de 2026-09-26 (regras novas)

Saídas da auditoria de código de 2026-09-26 (`CODE_AUDIT.md`, secção do topo).

- **Produção = o domínio e os seus subdomínios, num só sítio:** `isProductionHost()` em `src/constants/tracking.ts`, usado pelo `trackingEnv()`, pelo `IS_PRODUCTION` de `quizTracking.ts` e pelo `errorTracking.ts`. Antes só o domínio exato contava, e o `submissionService` **simula** o envio fora de produção (a pessoa vê o obrigado e nada é gravado nem enviado): se o `www` passasse a servir o site, todos os pedidos feitos lá desapareciam em silêncio. `*.pages.dev`, `localhost` e qualquer outro domínio continuam a ser pré-visualização. Isto também desfez o ciclo `errorTracking` ↔ `quizTracking`.
- **`send-lead-email`: as variáveis do Resend verificam-se antes de tomar o trinco** de `lead_notifications`, e todo o caminho que não termine num envio aceite larga-o. Antes, um 503 por variável em falta deixava o trinco tomado e a tentativa seguinte recebia `duplicate` — que o cliente trata como entregue. A verificação de injeção de cabeçalhos passou a olhar só para o assunto e o reply-to (sobre o JSON inteiro não apanhava nada e recusava pedidos com um "%0A" num URL).
- **reCAPTCHA a funcionar desde 2026-09-26.** A CSP bloqueava o script desde 23/09 (faltavam `www.google.com/recaptcha/` e `www.gstatic.com/recaptcha/` em `script-src`/`frame-src`), por isso o token saía vazio e o servidor deixava passar tudo: a proteção estava desligada sem ninguém dar por isso. As chaves estão configuradas (site no build do Cloudflare, secreta no Supabase). **Regra nova em `_shared/recaptcha.ts`: um problema nosso nunca recusa um pedido** (chave secreta que não corresponde, API da Google em baixo, token expirado — tudo isso deixa passar e fica registado); só se recusa um token validado com pontuação abaixo de 0.3 ou de outra ação. Um bot não ganha nada com a tolerância: já podia não mandar token. `RECAPTCHA_ENABLED=false` nos segredos desliga tudo sem publicar código. Ver `docs/lead-spam-protection.md`.
- **As Edge Functions têm testes e type-check:** `npm run test:functions` (Deno, instalado em `/opt/homebrew/bin/deno` nesta máquina) e `deno check supabase/functions/<nome>/index.ts` antes de publicar. Publicar com `node_modules/.bin/supabase functions deploy <nome>` a partir da raiz (o `npx -y supabase@latest` falhou nesta sessão com EACCES na cache do npm).
- **`20240101_admin_tables.sql` deixou de apagar a tabela `leads`.** Tinha `drop table if exists public.leads cascade` desde o commit inicial, e o `db push` de 14/09 reaplicou esse ficheiro. **Por confirmar pelo dono:** `select min(created_at), count(*) from public.leads;` no SQL Editor — se o mínimo for ≥ 2026-09-14, os leads anteriores perderam-se da base (os emails do Resend continuam a ter as cópias).
- **Faturação por serviço fora do repositório público.** `scripts/whatsapp-services-raw.txt` e o parser foram apagados; `.gitignore` ganhou `scripts/*-raw.txt`. Continuam no histórico do git (commit `c63d98c`): reescrever o histórico é decisão do dono.
- **Chunks de um deploy anterior:** `main.tsx` recarrega a página uma vez em `vite:preloadError` (guarda de 60 s na sessão). Sem o catch-all SPA, quem tinha uma página aberta durante um deploy ficava com a página em branco ao navegar ou ao abrir o quiz. O `key` da página no `App.tsx` usa o `pathname` do router, não o `location` global.
- **Páginas-pilar: uma só fonte, `src/data/pillarPages.ts`** (título, descrição, H1, FAQs, rótulo de preço) lida pelas seis páginas React, pelo `PageHead` (que a carrega a pedido para essas seis rotas; já não estão no `routeMeta`) e pelo `CORE` do `prerender.ts`. Antes eram três cópias: em `/limpeza-sofas` o HTML estático tinha outro H1 e outras 4 FAQs (com "1 a 3 horas" contra "45 minutos a 2 horas" no React), e a FAQ de preço das cadeiras só existia no estático, com escalões errados. As FAQs passaram a ser a união das duas listas. As listas de benefícios do estático ficam como estão (o dono confirmou que as afirmações estão certas). `pillarPages.test.ts` e `src/pages/PillarPages.test.tsx` rebentam se os três lados divergirem.
- **Cadeiras: o preço diz-se sempre por cadeira, nunca "desde"/"a partir de" (dono, 2026-09-26: "não digas desde em cadeiras").** "20€ por cadeira" (em títulos "20€ por Cadeira"; na impermeabilização "Essencial a 18€ por cadeira, Premium a 25€ por cadeira"). Qualquer rótulo de preço de partida passa por `startingPriceLabel()`/`CHAIR_PRICE_LABEL`/`perChairPrice()` em `enginePrices.ts`, que devolvem "Desde 49€" para os outros serviços e a forma por cadeira para as cadeiras. As páginas `/impermeabilizacao-cadeiras-*` têm subtítulo e pontos de confiança próprios (antes mostravam os do sofá).
- **Preços em texto saem de `src/data/enginePrices.ts`** (preços de partida lidos do motor, escalões das cadeiras a partir do `calcChairClean`, e `formatEuro`, com vírgula). Um preço novo numa página escreve-se com estas constantes, nunca com o número. Mantém-se sem imports `@/` para o prerender o ler.
- **JSON-LD de ofertas: `offerForPriceLabel()` em `seoSchema.ts`.** Sem número no rótulo não há `Offer` (antes, os componentes caíam para "49€" e todas as páginas de tapetes e alcatifas declaravam uma oferta de 49€ num serviço sempre sob orçamento; o colchão declarava 49€ em vez de 59€). Sem `priceValidUntil` escrito à mão (expirava a 2026-12-31).
- **Páginas de marca sem preços próprios.** Os intervalos por marca (Natuzzi 79–129€, Kave Home 59–89€, "Desde 12.5€" nas cadeiras…) eram inventados — o dono não os reconheceu — e foram apagados de `marca*Data.ts`; as páginas mostram o preço de partida do serviço. Título, H1, migalha, processo e "Outras marcas" (com o nome real da marca) vêm de `marcaPageCopy`/`otherMarcaLinks` em `marcaCities.ts`, lidos pelas páginas React e pelo prerender.
...
**Atualizado em 2026-09-24: a etiqueta da conversão do Google Ads já existe e já dispara.**
A nota anterior aqui dizia que faltava — deixou de ser verdade nalgum momento
entre 22 e 24/09, sem que este ficheiro fosse atualizado. Confirmado a submeter
um pedido a sério em produção: o `dataLayer` regista um evento `conversion` com
`send_to: "AW-18457115875/ewA0CLXn_fscEOP5hOFE"`, o `lead_id` e o valor certos.
**O que estava mesmo a impedir a conversão de chegar ao Ads era outra coisa,
já corrigida:** ver a nota da nona armadilha / secção CSP sobre `connect-src`
