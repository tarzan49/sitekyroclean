...
export default function CustomerReviews({ reviews }: { reviews: PoolReview[] }) {
  ...
  useEffect(()=>{
    ...
    return()=>{api.off('select',select);api.off('reInit',select);api.off('pointerDown',stop);};
  },[api]);
  useEffect(()=>{
  ...
  return <div ref={root}>
    <a href={GOOGLE_REVIEWS_VIEW_URL} target="_blank" rel="noopener noreferrer" className="inline-flex flex-wrap items-center gap-x-3 gap-y-2 bg-white rounded-sm border border-[#173629]/15 px-4 py-3 mb-5 text-[#173629]">
      <GoogleG className="w-6 h-6" /><span className="text-lg font-bold">{REVIEW_RATING}<span className="text-xs font-normal text-[#617166]"> / 5</span></span><span className="text-xs">{REVIEW_COUNT}+ avaliações no Google</span><ExternalLink className="w-3.5 h-3.5" />
    </a>
    <div ref={ref} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocusCapture={()=>setPaused(true)} className="overflow-hidden" role="region" aria-label="Testemunhos dos clientes" aria-roledescription="carrossel">
      <div className="flex items-stretch -ml-4">{reviews.map((review,i)=><div key={`${review.name}-${i}`} role="group" aria-roledescription="testemunho" aria-label={`${i+1} de ${reviews.length}`} className="shrink-0 grow-0 basis-[92%] sm:basis-1/2 lg:basis-1/3 pl-4 min-w-0"><CustomerReviewCard review={review} /></div>)}</div>
    </div>
    {reviews.length>1&&<div className="flex items-center justify-between gap-4 mt-5 bg-[#173629] border border-white/15 rounded-sm p-2 text-white">
      <p className="text-xs pl-2 tabular-nums">{selected+1} / {reviews.length}<span className="ml-2 text-white/70">Histórias de clientes</span></p>
