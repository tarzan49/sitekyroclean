      ...
      <div className="border border-[#173629]/15 bg-white rounded-sm overflow-hidden">
        <div className="grid grid-cols-5 border-b border-[#173629]/15" role="tablist" aria-label="Etapas da limpeza">
          {steps.map((item, i) => <button key={item.label} id={`${id}-tab-${i}`} role="tab" aria-selected={active === i} aria-controls={`${id}-panel`} tabIndex={active === i ? 0 : -1}
            ...
            <span className={`block text-sm mb-1 ${active === i ? 'text-gold' : 'text-[#917634]'}`}>0{i + 1}</span>
        ...
        <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${active}`} tabIndex={0} className="grid md:grid-cols-2 focus-visible:outline focus-visible:outline-gold">
          <div className="relative overflow-hidden bg-[#efeee7] min-h-[190px] md:min-h-[340px]" style={{ aspectRatio: '560 / 300' }}>
            ...
            <span className="absolute bottom-3 left-3 text-sm bg-white/90 px-2 py-1 text-[#536259]">Imagem ilustrativa</span>
          </div>
          <div className="p-5 sm:p-8 flex flex-col">
            <p className="text-sm uppercase tracking-[0.2em] text-[#94772b] mb-3">Etapa {active + 1} de {steps.length}</p>
            <h3 className="type-card-title font-playfair    text-[#173629] mb-3">{step.title}</h3>
            <p className="text-base sm:text-base text-[#536259] leading-relaxed min-h-[84px]">{step.description}</p>
            <div className="flex items-center justify-between gap-3 mt-auto pt-6">
              ...
              <button type="button" onClick={() => setActive(a => (a + 1) % steps.length)} className="min-h-12 flex items-center justify-center gap-3 px-5 bg-[#173629] text-white font-semibold text-base hover:bg-[#254535]">{active === steps.length - 1 ? 'Ver desde o início' : 'Próxima etapa'}<ArrowRight className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
      </div>
      <a href={image} download="Kyro-Clean-como-limpamos-o-seu-sofa.png" className="inline-flex items-center gap-2 min-h-11 mt-4 text-sm text-[#536259] underline underline-offset-4 hover:text-[#173629]"><Download className="w-4 h-4" />Guardar o resumo das cinco etapas</a>
    </div>
  </section>;
}
