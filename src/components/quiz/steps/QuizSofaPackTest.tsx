...
export default function QuizSofaPackTest({ base, travel, onChoose, onBack }: {
  ...
  return <div className="w-full max-w-sm mx-auto space-y-3 py-2 text-left">
    <p className="text-gold text-sm tracking-[0.22em] uppercase">Aproveite a mesma visita</p>
    <h2 className="type-quote-title font-playfair  text-white">Limpe também o seu colchão de casal</h2>
    <div className="p-4 border border-gold/40 bg-gold/5 rounded-sm space-y-3">
      <div className="flex justify-between gap-3 text-base text-white/80"><span>Numa visita separada</span><span className="line-through">{eur(separateVisit)}</span></div>
      <p className="text-sm text-white/80">Limpeza {eur(Number(mattress.cleaningPrice))} + deslocação {eur(travel)}</p>
      <div><p className="text-sm text-white/70">Acrescente ao pedido por apenas</p><p className="font-playfair text-5xl text-gold mt-1">+{eur(TEST_CASAL_ADDON_PRICE)}</p></div>
      <p className="inline-block bg-gold/15 text-gold font-bold text-base px-3 py-2 rounded-sm">Poupa {eur(saving)} face a uma visita separada</p>
      <p className="text-sm text-white/80">Preço especial na mesma visita. Sem uma segunda taxa de deslocação.</p>
      <div className="border-t border-white/15 pt-3 space-y-2 text-base text-white/80">
    ...
    <button className="w-full min-h-11 text-base text-white/75 tracking-wide  underline" onClick={() => onChoose(null)}>Continuar só com o sofá por {eur(base + travel)}</button>
    <button className="w-full min-h-11 text-base text-white/80" onClick={onBack}>Voltar aos tratamentos</button>
  </div>;
}
