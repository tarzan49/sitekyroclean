  ...
  return (
    <div className="flex w-full flex-col items-center gap-3">
      <div className="space-y-1.5 text-center">
        <h2 className="type-quote-title font-playfair    text-white">{carpetKind === 'alcatifa' ? 'Que áreas de alcatifa vamos limpar?' : 'Que tapetes vamos limpar?'}</h2>
      </div>

      <details className="w-full max-w-sm rounded-xl border border-[#D4AF37]/20 bg-[#D4AF37]/[0.06] px-3">
        <summary className="min-h-11 cursor-pointer py-3 text-left text-sm font-medium text-[#E8D58F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37] rounded-lg">Comprimento × largura <span className="font-normal text-white/70">· Como medir?</span></summary>
      ...
      <div className="flex w-full max-w-sm flex-col gap-2">
        ...
        <button type="button" onClick={() => setCarpetItems(prev => carpetAddItem(prev))} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#D4AF37]/50 px-3 py-2.5 text-base font-semibold text-[#E8D58F] transition-colors hover:bg-[#D4AF37]/10 hover:border-[#D4AF37] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37] touch-manipulation">
