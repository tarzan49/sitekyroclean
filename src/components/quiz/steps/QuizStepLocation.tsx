...
const QuizStepLocation = ({ location, locationQuery, setLocationQuery, scrollContainerRef, onCitySelect }: QuizStepLocationProps) => {
  ...
  return (
    <div className="w-full max-w-sm mx-auto text-left py-3 sm:py-1 pr-4 sm:pr-0">
      <p className="text-gold text-[10px] font-bold tracking-[0.25em] uppercase mb-3">O SEU ORÇAMENTO</p>
      <h2 className="font-playfair text-[2rem] sm:text-4xl font-bold text-white leading-[1.1] mb-3">Vamos até si.</h2>
      <p className="text-sm text-white/65 leading-relaxed mb-6">Confirme a localidade onde pretende o serviço. Nós tratamos do resto.</p>

      {loading && <div role="status" className="flex items-center gap-3 p-4 mb-4 border border-gold/20 rounded-xl text-sm text-white/75"><Loader2 className="w-5 h-5 text-gold animate-spin shrink-0" /><span>A encontrar a sua localidade…<span className="block text-xs text-white/55 mt-1">Permita a localização no navegador ou pesquise abaixo.</span></span></div>}
      {message && <p role="status" className="text-sm text-white/70 mb-4 leading-relaxed">{message}</p>}

      {selected && <div className="rounded-xl border border-gold/50 bg-gold/[0.07] p-4 mb-3">
        <div className="flex items-center gap-2 text-[10px] tracking-[0.13em] uppercase text-gold mb-3"><Navigation className="w-3.5 h-3.5" />{selected === detected ? 'Localidade detetada' : 'Localidade do serviço'}</div>
        ...
        <button type="button" onClick={() => { stopDetection(); setEditing(!editing); setLocationQuery(''); }} className="text-sm text-white/70 underline underline-offset-4 min-h-11 mt-1">{editing ? 'Manter esta localidade' : 'Alterar localidade'}</button>
      </div>}

      {editing && <div className="mb-4 space-y-2">
        <label htmlFor="quiz-location-search" className="block text-xs text-white/65 mb-2">{selected ? 'Onde pretende o serviço?' : 'Localidade do serviço'}</label>
        <input ref={input} id="quiz-location-search" type="search" placeholder="Pesquisar localidade" value={locationQuery} onChange={event => { stopDetection(); setLocationQuery(event.target.value); }} onFocus={() => { if (scrollContainerRef.current) scrollContainerRef.current.scrollTop = 0; }} autoComplete="off" className="w-full h-12 px-4 text-base bg-[#1a2a1a] border border-gold/25 focus:border-gold rounded-xl text-white placeholder:text-white/45" />
        {!locationQuery && regional.length > 0 && <p className="pt-2 text-xs text-white/55">Outras localidades na sua região</p>}
        {matches.length > 0 && <div className="border border-white/10 rounded-xl overflow-hidden bg-[#1a2a1a]">{matches.map(city => <button type="button" key={city} onClick={() => choose(city)} className="w-full min-h-12 flex justify-between items-center gap-3 px-4 py-3 text-left text-sm text-white border-b border-white/5 last:border-0 hover:bg-gold/10">{city}<ChevronRight className="w-4 h-4 text-white/40 shrink-0" /></button>)}</div>}
        {locationQuery && matches.length === 0 && <p className="text-sm text-white/60 py-2">Localidade não encontrada. Tente o nome do concelho.</p>}
        {!loading && <button type="button" onClick={() => { manuallyEdited.current = false; setAttempt(value => value + 1); }} className="min-h-11 text-sm text-gold flex items-center gap-2"><Navigation className="w-4 h-4" />Usar a minha localização</button>}
      </div>}
