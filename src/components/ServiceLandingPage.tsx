...
import ServiceLeadActions from "@/components/ServiceLeadActions";
import { LANDING_HERO } from "@/constants/landingLayout";
...
import { getServiceLandingData } from "@/data/serviceLandingData";
...
const ServiceLandingPage = () => {
  ...
  const data = useMemo(() => getServiceLandingData(location.pathname), [location.pathname]);
  const [selectedService, setSelectedService] = useState("limpeza-sofas");
  ...
  const activeService = data.generalService ? selectedService : data.serviceSlug;
  const quizService = data.quizService ?? SERVICE_TO_QUIZ[activeService];
  ...
  const waUrl = `${WHATSAPP_BASE}?text=${encodeURIComponent(data.waMessage ?? buildServiceWaMessage(data.generalService ? "limpeza-estofos" : data.serviceSlug, data.parentPlace ? `${data.city}, ${data.parentPlace}` : data.city))}`;

  // "Porto" takes the definite article ("no Porto"); every other city in our list reads naturally with "em"
  const cityPrep = data.city === 'Porto' ? 'no' : 'em';
  const travelFee = locationPrices[data.quoteCity];
  const travelText = data.coverageNote || travelFee === undefined ? 'Deslocação confirmada pela morada, sob consulta.' : `Deslocação +${travelFee}€ a ${data.city}.`;
  ...
  const problemCards = data.problems.map((problem, idx) => ({
    ...
    image: problemImages[idx % problemImages.length],

  })).filter(card => card.image);


  return (
    <QuizLocationProvider value={data.quoteCity}>
    <QuizServiceProvider value={quizService}>
      ...
      <main>
        ...
        <section className={LANDING_HERO.section}>
          <div className="container mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
            <div className={LANDING_HERO.grid}>
              <div>
                <PageBreadcrumb items={data.breadcrumbs} />
                ...
                {data.coverageNote && <p className="text-white/85 text-sm leading-relaxed mb-4">{data.coverageNote}</p>}
                <ServiceLeadActions city={data.coverageNote ? undefined : data.quoteCity} price={data.priceFrom} href={waUrl} source={`${data.family}_hero_${data.serviceSlug}_${data.locationSlug}`} />
              </div>

              <div id="resultados" className={LANDING_HERO.media}>
              ...
              <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
                <div>
                  <SectionHeader
                    ...
                    goldWord={data.city}
                    subtitle={`${data.serviceSlug === 'limpeza-tapetes' ? 'Orçamento à medida de cada tapete.' : data.serviceSlug === 'limpeza-alcatifas' ? 'Orçamento à medida de cada espaço, sem preço fixo por m².' : 'Estimativa confirmada antes da marcação.'} ${travelText} Orçamento gratuito antes de qualquer compromisso.`}
                  />
                  {/* Trust facts — desktop only (variante 1) */}
                  <div className="hidden md:block">
                    <ServiceTrustDesktop serviceSlug={activeService} variant={1} seedKey={data.city} />
                  </div>
                </div>
                <div>
                  {data.generalService && <label className="block mb-5 text-sm font-medium">Escolha o serviço
                    <select className="block w-full mt-2 p-3 border border-[#173629]/25 bg-white" value={selectedService} onChange={event => setSelectedService(event.target.value)}>
                      {services.map(service => <option key={service.slug} value={service.slug}>{service.name}</option>)}
                    </select>
                  </label>}
                  <PriceWidget key={activeService} serviceSlug={activeService} initialLocation={data.quoteCity} />
                </div>
              </div>
              {/* Trust mobile colapsável — abaixo do widget */}
              <div className="lg:hidden">
                <ServiceTrustMobile serviceSlug={activeService} variant={1} seedKey={data.city} />
              </div>
            </div>
          </section>
        )}

        {data.family === "price" && <PriceFactors serviceSlug={data.serviceSlug} />}

        <QuizFormLazy
          isOpen={isProblemQuizOpen}
          onClose={closeProblemQuiz}
          initialLocation={data.quoteCity}
        ...
        <section id="avaliacoes" className="scroll-mt-6 py-14 md:py-20 bg-kyro-green">
          <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
            ...
            <ServiceReviewsGrid serviceSlug={data.serviceSlug} seed={`${data.quoteCity}-${data.locationSlug}`} heading="" />
          </div>
        </section>
        ...
        <ServiceProcessGuide serviceSlug={activeService} city={data.city} cityPrep={cityPrep} />
        ...
        <section className="py-14 md:py-20" style={{ backgroundColor: "#FDFDF9" }}>
          <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
            ...
            <div className="max-w-3xl text-sm sm:text-base text-[#536259] leading-relaxed space-y-3 mb-8">
              <p>{data.intro}</p>
              <p>{data.howItWorks}</p>
              <p>{data.coverageNote ?? data.localSection}</p>
            </div>
