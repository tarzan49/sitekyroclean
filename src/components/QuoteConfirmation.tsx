  ...
  return (
    <div className="min-h-svh bg-[#071a12] px-4 pb-4 text-white sm:px-6">
      <main className="mx-auto w-full max-w-md">
        <header className="flex h-14 items-center justify-between border-b border-[#D4AF37]/20">
        ...
        <h1 className="py-4 text-center text-[28px] font-semibold leading-[1.12] tracking-[-0.04em]">
          {receipt ? 'Obrigado pela confiança.' : 'O próximo passo é simples.'}
          <span className="mt-1 block text-[#D4AF37]">Falamos no WhatsApp?</span>
        </h1>

        <section aria-labelledby="receipt-title" className="overflow-hidden rounded-2xl border border-[#D4AF37]/35 bg-[#f7f5ef] text-[#132c22] shadow-[0_16px_48px_#00000030]">
          <div className="h-0.5 bg-gradient-to-r from-[#D4AF37]/20 via-[#D4AF37] to-[#D4AF37]/20" />
          <div className="flex items-center justify-between gap-3 px-4 pb-3 pt-4">
            <h2 id="receipt-title" className="text-[11px] font-bold uppercase tracking-[0.2em]">Resumo do pedido</h2>
            {receipt?.bookingId && <span className="font-mono text-[10px] tracking-wide text-[#627165]">#{receipt.bookingId}</span>}
          </div>

          {receipt ? (
            <>
              {(receipt.location || (receipt.slot && receipt.slot !== 'Não especificado')) && (
                <div className="mx-4 flex flex-wrap gap-x-3 gap-y-1 border-b border-[#132c22]/10 pb-3 text-[11px] leading-relaxed text-[#526458]">
              ...
              <ul className="divide-y divide-[#132c22]/10 px-4">
                {receipt.lines.map((line, index) => (
                  <li key={index} className="flex items-center justify-between gap-3 py-3">
                    <div className="flex min-w-0 items-start gap-2.5">
                      <span aria-label={`Quantidade: ${line.qty}`} className="mt-0.5 flex h-5 min-w-5 shrink-0 items-center justify-center rounded border border-[#132c22]/15 px-1 text-[10px] font-semibold text-[#526458]">{line.qty}×</span>
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium leading-snug">{line.label}</p>
                        {line.qty > 1 && line.unitPrice !== null && <p className="mt-0.5 text-[10px] text-[#627165]">{money(line.unitPrice)} / un.</p>}
                      </div>
                    </div>
                    <span className="shrink-0 text-[13px] font-semibold tabular-nums">{money(line.total)}</span>
                  </li>
                ))}
              </ul>

              <div className="border-t border-[#132c22]/10 bg-[#eeeee5] px-4 py-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="text-xs font-medium">{receipt.sobOrcamento ? 'Subtotal conhecido' : 'Total estimado'}</span>
                  <strong className="text-[30px] font-semibold leading-none tracking-[-0.045em] tabular-nums">{money(receipt.total)}</strong>
                </div>
                <p className="mt-2 text-[10px] leading-relaxed text-[#627165]">{receipt.sobOrcamento ? 'Acrescem os serviços sob orçamento. Valor final a confirmar.' : 'Valor e marcação sujeitos a confirmação.'}</p>
              </div>
            </>
          ) : <p className="px-4 pb-4 text-sm leading-relaxed text-[#526458]">Os detalhes do pedido não estão disponíveis nesta sessão. Fale connosco para o localizar.</p>}
        </section>

        <div className="mt-4">
          <a href={waUrl} target="_blank" rel="noopener noreferrer" data-tracking-source="quote_confirmation" className="flex min-h-14 items-center justify-center gap-2.5 rounded-xl border border-white/15 bg-gradient-to-b from-[#39de78] to-[#25D366] px-3 text-[15px] font-bold text-[#071a12] shadow-[0_6px_24px_#25d36620] transition-colors hover:from-[#59e890] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#D4AF37]">
          ...
          <p className="mt-2 flex items-center justify-center gap-1.5 text-[10px] text-white/65"><Clock className="h-3 w-3 text-[#D4AF37]" />{RESPONSE_PROMISE}</p>
        </div>

        <a href={GOOGLE_REVIEWS_VIEW_URL} target="_blank" rel="noopener noreferrer" aria-label={`${REVIEW_RATING} de 5 estrelas, mais de ${REVIEW_COUNT} avaliações no Google. Ler avaliações`} className="mx-auto mt-4 flex min-h-11 w-fit items-center justify-center gap-3 rounded-sm outline-offset-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#D4AF37]">
          ...
          <span>
            ...
            <span className="mt-1 block text-[10px] text-white/70">{REVIEW_COUNT}+ avaliações Google</span>
          </span>
        </a>
      </main>
    </div>
