...
export default function SofaLeadActions({ city, price, href, source }: { city: string; price: string; href: string; source: string }) {
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="text-white/85 text-xs leading-relaxed">Desde {price} + deslocação {fee === undefined ? 'a partir de 10€ (confirmada pela morada)' : `de ${fee}€`}. Orçamento gratuito, sem compromisso.</p>
    </div>
  );
}
