...
import { getServiceProcess } from '@/constants/serviceProcessGuides';

const sofaImage = '/images/services/sofa-cleaning-process-guide.png';
const sofaSteps = [
  ...
  { label: 'Secagem', title: 'Depois, é deixar o tecido secar.', description: 'Normalmente demora 3 a 6 horas, conforme o tecido e a ventilação. Use o sofá apenas quando estiver completamente seco.', y: 1124, height: 270, alt: 'Sofá junto de uma janela aberta durante a secagem' },
];

export default function ServiceProcessGuide({ serviceSlug = 'limpeza-sofas', city, cityPrep = 'em' }: { serviceSlug?: string; city?: string; cityPrep?: string }) {
  const config = getServiceProcess(serviceSlug);
  const steps = config?.steps ?? sofaSteps;
  const image = sofaImage;
  const [active, setActive] = useState(0);
  const id = useId();
  const step = steps[active];
  return <section id="processo" className="scroll-mt-20 py-14 md:py-20 bg-[#FDFDF9]">
    <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
      <SectionHeader overline="Como funciona" heading={`${config?.heading ?? "A limpeza do seu sofá"}, passo a passo${city ? ` ${cityPrep}` : ""}`} goldWord={city ?? ""} subtitle="Explore as cinco etapas e os cuidados de cada fase do serviço." />
      <div className="border border-[#173629]/15 bg-white rounded-sm overflow-hidden">
        ...
        <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${active}`} tabIndex={0} className="grid md:grid-cols-2 focus-visible:outline focus-visible:outline-gold">
          <div className="relative overflow-hidden bg-[#efeee7] min-h-[190px] md:min-h-[340px]" style={{ aspectRatio: '560 / 300' }}>
            {config ? <img src={config.images[active]} alt={`${config.subject}: ${step.label}`} loading="lazy" className="absolute inset-0 w-full h-full object-cover" /> : <div key={step.label} className="absolute inset-x-0 top-1/2 -translate-y-1/2 scale-150 overflow-hidden" style={{ aspectRatio: `560 / ${sofaSteps[active].height}` }}>
              <img src={image} alt={sofaSteps[active].alt} className="absolute !max-w-none" style={{ width: '182.8572%', left: '-4.6429%', top: `${-sofaSteps[active].y / sofaSteps[active].height * 100}%` }} />
            </div>}
      ...
      {!config && <a href={image} download="Kyro-Clean-como-limpamos-o-seu-sofa.png" className="inline-flex items-center gap-2 min-h-11 mt-4 text-xs text-[#536259] underline underline-offset-4 hover:text-[#173629]"><Download className="w-4 h-4" />Guardar o resumo das cinco etapas</a>}
    </div>
  </section>;
}
