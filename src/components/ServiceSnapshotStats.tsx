...
const ServiceSnapshotStats = ({ stats }: { stats: SnapshotStat[] }) => {
  return (
    <section style={{ backgroundColor: "#071a12" }} className="border-b border-white/5">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        <div
          className="grid grid-cols-2 md:grid-cols-4"
          style={{ gap: "1px", backgroundColor: "rgba(255,255,255,0.07)" }}
        >
          {stats.map((s, i) => (
            <div
              key={i}
              className="py-7 md:py-9 px-3 text-center"
              style={{ backgroundColor: "#071a12", borderTop: "2px solid rgba(212,175,55,0.55)" }}
            >
