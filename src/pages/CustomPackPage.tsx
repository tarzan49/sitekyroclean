...
export default function CustomPackPage() {
  ...
  const msg = [`Olá! Gostaria de confirmar este pack personalizado em ${city}:`, ...prices.lines.map(l => `${l.label}: ${l.amount === null ? 'sob orçamento' : money(l.amount)}${l.quote && l.amount !== null ? ' + extra sob orçamento' : ''}`), `Serviços tabelados: ${money(prices.subtotal)}`, ...(prices.discountActive ? [`Desconto Pack Família: ${money(prices.discount)}`] : []), `Deslocação: ${prices.travel === null ? 'a confirmar' : money(prices.travel)}`, `${prices.quote ? 'Subtotal conhecido' : 'Estimativa total'}: ${money(prices.total)}${prices.quote ? ' + valores sob orçamento' : ''}`, 'Podem confirmar o valor e a disponibilidade?'].join('\n');
  return <><Header /><main className="bg-[#FDFDF9] text-[#111111] pt-28 pb-16"><div className="max-w-6xl mx-auto px-5">
    <p className="text-[#8D7020] uppercase tracking-widest text-xs mb-4">Uma visita, os artigos que escolher</p><h1 className="font-playfair text-4xl md:text-5xl mb-5">{preset ? `${preset.pack.name} em ${preset.city.name}` : 'Monte o seu pack de limpeza'}</h1>
    ...
    <div className="grid lg:grid-cols-[1fr_360px] gap-7 items-start"><section aria-label="Configurar artigos" className="space-y-5">
      ...
      {items.map((item, idx) => <article key={item.id} className="bg-white border border-[#E5E0D3] rounded-xl p-5 space-y-4"><div className="flex items-center justify-between gap-4"><h2 className="font-playfair text-2xl">{idx + 1}. {PACK_KIND_LABEL[item.kind]}</h2><button type="button" aria-label={`Remover artigo ${idx + 1}`} onClick={() => setItems(old => old.filter(i => i.id !== item.id))} className="p-3"><Trash2 className="w-5 h-5" /></button></div>
    ...
    </section><aside className="bg-[#071a12] text-white rounded-xl p-6 lg:sticky lg:top-24"><h2 className="font-playfair text-2xl mb-5">O seu pack</h2><div className="space-y-4 text-sm">{prices.lines.map((line, i) => <div key={items[i].id} className="border-b border-white/15 pb-3"><p className="text-white/75">{line.label}</p><p className="font-semibold mt-1">{line.amount === null ? 'Sob orçamento' : money(line.amount)}{line.quote && line.amount !== null ? ' + extra sob orçamento' : ''}</p></div>)}<p>Serviços tabelados: {money(prices.subtotal)}</p>{prices.discountActive && <p className="text-gold">Desconto Pack Família (10%): −{money(prices.discount)}</p>}<p>Deslocação: {prices.travel === null ? 'A confirmar' : money(prices.travel)}</p><p className="text-lg font-semibold">{prices.quote ? 'Subtotal conhecido' : 'Estimativa total'}: {money(prices.total)}</p>{prices.quote && <p className="text-gold">Acrescem os artigos ou extras sob orçamento.</p>}</div>
      <p className="text-xs text-white/65 mt-5">Desconto de 10% nos serviços quando a soma tabelada ultrapassa 149€, com pelo menos dois artigos e um deles a partir de 49€. Uma deslocação, sem desconto. Valores sob orçamento confirmados separadamente.</p>
      {prices.valid ? <a href={`${WHATSAPP_BASE}?text=${encodeURIComponent(msg)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 mt-6 bg-gold text-[#071a12] rounded-lg p-4 font-bold"><MessageCircle className="w-5 h-5" />Confirmar o meu orçamento</a> : <p role="status" className="mt-6 rounded-lg bg-white/10 p-4">Escolha a localidade e complete quantidades e medidas válidas para pedir a confirmação.</p>}<p className="text-xs text-white/70 mt-4">Resposta em menos de 10 minutos</p></aside></div>
    <section className="mt-14 max-w-3xl space-y-4"><h2 className="font-playfair text-3xl">Antes de marcar</h2><p>{PRICE_PROMISE}</p><p>{AVAILABILITY_PROMISE}</p><p>{DRYING_PROMISE}</p><p>{SATISFACTION_PROMISE}</p><p>Impermeabilização Premium: até 10 anos, conforme utilização e cuidados. Essencial: 1 a 2 anos. A cura do tratamento pode exigir 24 horas; confirme as instruções da equipa.</p><div className="flex flex-wrap gap-5 pt-4"><Link className="underline" to="/tratamento-anti-acaros">Conhecer o tratamento anti-ácaros</Link><Link className="underline" to="/desbacterizacao">Conhecer a desbacterização</Link><Link className="underline" to="/guia-de-packs">Packs por localidade</Link></div></section>
  </div></main><Footer /></>;
}
