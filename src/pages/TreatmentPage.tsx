...
export default function TreatmentPage() {
  ...
  return <><Header /><main className="bg-[#FDFDF9] text-[#111111]">
    <section className="bg-[#071a12] text-white pt-28 pb-16 px-5"><div className="max-w-5xl mx-auto">
      <p className="text-gold uppercase tracking-widest text-sm mb-5">Cuidado à medida dos seus estofos</p>
      <h1 className="type-page-title font-playfair   max-w-4xl mb-6">{page.h1}</h1>
      ...
      <p className="text-base text-white/70 my-4">Resposta em menos de 10 minutos · Sem compromisso</p><TrustRatingBadge variant="horizontal" />
    </div></section>
    <section className="max-w-5xl mx-auto px-5 py-14"><h2 className="type-section-title font-playfair  mb-8">O que pode acrescentar ao seu cuidado habitual</h2><div className="grid md:grid-cols-3 gap-5">{page.benefits.map(b => <article key={b} className="p-6 bg-white border border-[#E8E4DE] rounded-xl"><CheckCircle2 className="text-[#9B7D20] mb-4" /><p>{b}</p></article>)}</div><p className="mt-8 leading-relaxed">{page.detail}</p></section>
    <section className="bg-[#F0EEE7] px-5 py-12"><div className="max-w-5xl mx-auto"><h2 className="type-section-title font-playfair  mb-5">Uma proposta clara, antes de marcar</h2><p className="mb-4">{PRICE_PROMISE}</p><p>{page.coverage}</p></div></section>
    <ServiceFAQ faqs={page.faqs} includeSchema={false} />
    <section className="max-w-5xl mx-auto px-5 pb-16"><h2 className="type-section-title font-playfair  mb-5">Escolha os artigos e os tratamentos</h2><div className="flex flex-wrap gap-3">{services.map(s => <Link className="border rounded-lg p-3" key={s.slug} to={`/${s.slug}${page.city ? `-${page.city.slug}` : ''}`}>{s.name}</Link>)}{treatments.map(t => <Link className="border rounded-lg p-3" key={t.slug} to={`/${t.slug}${page.city ? `-${page.city.slug}` : ''}`}>{t.name}</Link>)}<Link to="/packs" className="border rounded-lg p-3">Montar o meu pack</Link></div>
