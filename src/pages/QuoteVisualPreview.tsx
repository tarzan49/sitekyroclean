...
export default function QuoteVisualPreview() {
  ...
  const service = example === 'antiacaros' ? 'mattress' : (example === 'cadeiras' || example === 'quantidade-cadeiras' || example === 'protecao-cadeiras') ? 'chairs' : example === 'tapetes' ? 'carpet' : 'sofa';
  ...
  return <>
    ...
    <QuizForm isOpen={open} onClose={() => setOpen(false)} {...(fromStart ? {} : { initialLocation: "Porto",
      initialService: service, initialServiceType: original ? 'both' : (example === 'higienizacao' || example === 'protecao-cadeiras') ? 'waterproofing' : 'cleaning',
      ...
      initialWaterproofingTier: 'premium' as const, skipToUpsell: service !== 'carpet' && example !== 'quantidade-cadeiras' && example !== 'protecao-cadeiras' })} />
  </>;
}
