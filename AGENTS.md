...
Biblioteca visual de sofás concluída em `docs/sofa-image-pilot/`: 40 WebP, dez por problema, prompts/textos alternativos no manifesto e painel mobile com seleção independente. Ainda não está ligada às páginas públicas. Não inserir nos comparadores antes/depois. Faltam 200 imagens dos outros serviços e a integração com seleção estável nas páginas. `scripts/audit-sofa-image-library.mjs` verifica os 40 ficheiros e dados do painel. Detalhes no README da biblioteca; esta atualização substitui as notas de imagens pendentes para sofás acima.
