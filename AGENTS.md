...
Piloto visual de sofás criado em `docs/sofa-image-pilot/`: quatro WebP, um por problema, prompts no manifesto e painel mobile autónomo. São ilustrações de IA, não trabalhos reais; ainda não estão ligados às páginas públicas. Não os inserir nos comparadores antes/depois. Falta validar a direção visual e criar as restantes alternativas antes da expansão. Detalhes no README do piloto.
