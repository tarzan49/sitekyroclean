...
const entries = JSON.parse(fs.readFileSync('docs/waterproofing-image-library/prompts.json', 'utf8'));
assert.equal(entries.length, 40);
const hashes = new Set();
let bytes = 0;
for (const entry of entries) {
  assert.match(entry.file, /^protecao-(cafe|vinho|sumo|patas)-(0[1-9]|10)\.webp$/);
  assert.equal(entry.file, `${entry.id}.webp`);
  const groups = ['cafe', 'vinho', 'sumo', 'patas'];
  assert.equal(entry.problemId, `impermeabilizacao-${groups.indexOf(entry.id.split('-')[1]) + 1}`);
  ...
  const buffer = fs.readFileSync(`public/images/landing-problems/impermeabilizacao/${entry.file}`);
...
for (let index = 1; index <= 4; index++) assert.equal(entries.filter(entry => entry.problemId === `impermeabilizacao-${index}`).length, 10);
