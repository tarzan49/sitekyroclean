...
const entries = JSON.parse(fs.readFileSync('docs/alcatifa-image-library/prompts.json', 'utf8'));
assert.equal(entries.length, 40);
const hashes = new Set();
let bytes = 0;
for (const entry of entries) {
  assert.match(entry.file, /^alcatifa-(passagem|cafe|rodapes|escadas)-(0[1-9]|10)\.webp$/);
  assert.equal(entry.file, `${entry.id}.webp`);
  const groups = ['passagem', 'cafe', 'rodapes', 'escadas'];
  assert.equal(entry.problemId, `limpeza-alcatifas-${groups.indexOf(entry.id.split('-')[1]) + 1}`);
  ...
  const buffer = fs.readFileSync(`public/images/landing-problems/alcatifas/${entry.file}`);
...
for (let index = 1; index <= 4; index++) assert.equal(entries.filter(entry => entry.problemId === `limpeza-alcatifas-${index}`).length, 10);
