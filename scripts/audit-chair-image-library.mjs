...
const entries = JSON.parse(fs.readFileSync('docs/chair-image-library/prompts.json', 'utf8'));
assert.equal(entries.length, 40);
const hashes = new Set();
let bytes = 0;
for (const entry of entries) {
  assert.match(entry.file, /^cadeira-(residuos|manchas|odores|desgaste)-(0[1-9]|10)\.webp$/);
  assert.equal(entry.file, `${entry.id}.webp`);
  const groups = ['residuos', 'manchas', 'odores', 'desgaste'];
  assert.equal(entry.problemId, `limpeza-cadeiras-${groups.indexOf(entry.id.split('-')[1]) + 1}`);
  ...
  const buffer = fs.readFileSync(`public/images/landing-problems/cadeiras/${entry.file}`);
...
for (let index = 1; index <= 4; index++) assert.equal(entries.filter(entry => entry.problemId === `limpeza-cadeiras-${index}`).length, 10);
