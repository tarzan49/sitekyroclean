...
export function renderLandingPageHtml(model: LandingPageModel): string {
  const sections = {
    precos: `<h2>${e(model.priceHeading)}</h2><p>${e(model.pricingDescription)}</p>${model.variantExplanation ? `<p>${e(model.variantExplanation)}</p>` : ''}<ul>${model.priceRows.map(row => `<li>${e(row.item)}: ${e(row.price)}</li>`).join('')}</ul><p>${e(model.pricePolicy)}</p>${model.priceFactors.length ? `<details><summary>Como é calculado o preço?</summary>${model.priceFactors.map(factor => `<article><h3>${e(factor.title)}</h3><p>${e(factor.description)}</p><ul>${factor.examples.map(example => `<li>${e(example)}</li>`).join('')}</ul></article>`).join('')}</details>` : ''}`,
    ...
    problemas: `<h2>${e(model.problemHeading)}</h2>${model.problems.map(problem => `<article data-problem-id="${e(problem.id)}"><h3>${e(problem.title)}</h3><p>${e(problem.description)}</p></article>`).join('')}`,
