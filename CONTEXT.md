...
A seleção de imagens recebeu uma mistura final do hash para equilibrar as alternativas. Esta correção redistribui uma vez as escolhas anteriores; continuam estáveis por URL e idênticas no React/prerender. `scripts/audit-landing-seo.mjs` verifica metadados, títulos, conteúdo editorial, confiança, municípios e distribuição após o build. Resultado: 12.912 páginas sem falhas, 1.973 testes aprovados. Relatório: `docs/landing-seo-final-review.md`. Trabalho no ramo de pré-visualização, sem confirmação de publicação em produção.

## Bibliotecas visuais completas das landing pages (13/09/2026)

`alcatifaProblemImages.ts` e `waterproofingProblemImages.ts` completam os seis serviços: 240 imagens ilustrativas, 40 por serviço, dez por problema. Ficheiros novos em `public/images/landing-problems/alcatifas/` e `impermeabilizacao/`; prompts e revisão em `docs/alcatifa-image-library/` e `docs/waterproofing-image-library/`. A seleção partilhada mantém as escolhas anteriores, usa quatro imagens estáveis por URL e entrega as mesmas imagens no React e no HTML inicial. Alcatifas sob orçamento; proteção só para sofás e cadeiras. Não muda as quatro FAQ nem as galerias reais ou famílias autónomas. Os dois testes de dados percorrem 4.304 rotas; `LandingRemainingImages.test.tsx` cobre oito combinações serviço/família. Auditorias binárias próprias verificam dez alternativas por problema, dimensões e duplicados. Etapa integrada no ramo de pré-visualização, sem confirmação de publicação em produção.

## Galerias ampliáveis e tratamentos por problema (13/09/2026)

`VisualExamplesGallery` é agora usado nas quatro famílias de `LandingServiceSections`, além de materiais e problemas. Quatro imagens em 2×2 no mobile; descrição e pedido de avaliação no diálogo, conservando serviço e município. `ProblemExamplesGallery` liga problemas à biblioteca visual por serviço. `problemTreatmentGuides.ts` contém perfis explícitos por problema; `ProblemTreatmentGuide` substitui o guia genérico nas páginas nacionais e locais pela navegação por separadores de `IllustratedProcessGuide`, com cinco etapas nas limpezas (tratar → escovar → extrair), quatro na proteção/avaliação de bolor, fotografia, explicação e botões anterior/próxima. Prerender usa os mesmos textos e exemplos. Imagens novas locais reutilizadas, ficheiros antigos preservados, download discreto sem duplicar imagens. Detalhes, prompts e limites em `docs/problem-visual-refresh/README.md`.
...
## Imagens de problemas nas landing pages de sofás (13/09/2026)

`landingProblemImages.ts` contém as 40 imagens aprovadas, agrupadas pelos quatro IDs de problemas. `selectLandingProblemImage` usa a identidade da rota normalizada e do problema para pontuar cada alternativa; ignora query/hash. `landingPageModel.ts` inclui a imagem escolhida em cada problema, fonte única de React e `landing-page-html.ts`. Os 40 WebP estão em `public/images/landing-problems/sofas/`; as cópias de revisão em `docs/sofa-image-pilot/assets` são preservadas e comparadas nos testes. Só `limpeza-sofas` nas quatro famílias usa esta biblioteca; outros serviços e famílias continuam inalterados. Legenda «Imagem ilustrativa», dimensões 1200 × 675 e carregamento lazy. O antigo parâmetro DEV `teste=imagens-sofas` já não altera as imagens. Foram verificadas 2.152 páginas de sofás, uso das 40 imagens e igualdade de dados/HTML inicial; os controlos manuais do painel de revisão não são usados no site.
...
O prerender já consome esses mesmos geradores. `scripts/audit-landing-faqs.mjs` compara as quatro perguntas e respostas com os HTMLs e schemas de todas as rotas após o build; `--inventory-only` apresenta contagens e conjuntos distintos de perguntas. `scripts/landing-faq-routes.ts` delimita o inventário às quatro famílias. Preços e variantes removem apenas o FAQPage estático quando montam a versão cliente, via `clearPrerenderedFaqSchema`, preservando os outros metadados. A uniformização das secções está implementada; as 240 imagens e a expansão editorial continuam pendentes. Plano em `docs/seo-landing-diversity.md`.

## Composição partilhada das landing pages (13/09/2026)

`LandingServiceSections.tsx` é usado por `LocationServicePage`, `FreguesiaServicePage`, `PricePage` e `SofaVariantPage`, depois dos heroes/indicadores existentes. Ordem em `LANDING_SECTION_ORDER`: preços, avaliações, problemas, dúvidas, processo, mesma visita, zonas. Entradas Ads de sofás preservam a exceção de não mostrar diretório. Nenhuma outra família usa este novo componente.

`src/data/landingPageModel.ts` resolve apenas as quatro famílias, reutiliza os geradores de FAQ/metadados existentes e fornece dados puros ao React e a `scripts/landing-page-html.ts`. O `emit` do prerender seleciona este corpo apenas para rotas reconhecidas; as restantes conservam o gerador anterior. Avaliações continuam a usar `pickReviewSubset`, com as mesmas sementes, regiões e transcrições. Diretórios e packs usam destinos validados e o município, não o nome composto da variante de freguesia.
...
## Sessão — SEO turístico em inglês (/en/) + correções ao vivo em /problemas (2026-08-06, continuação)

Retomado o trabalho de SEO em inglês para turistas, com o user a corrigir o rumo em tempo real antes de eu avançar muito, e depois a reportar (e eu a corrigir imediatamente) vários bugs reais no redesign de `/problemas` da sessão anterior, enquanto ele próprio testava ao vivo.

**Correção de rumo do SEO EN (antes de escrever código):** a primeira versão do plano pesava as páginas por dados de turismo do Reino Unido (Algarve dominante por ser o maior mercado UK), o que o user rejeitou com força: "porque raio e que so te tas a focar nos britanicos? os turistas sao americanos ingleses franceses espanhois italianos chines etc." e "lisboa tem milhares de ALs... o algarve e turismo durante o verao, o porto e lisboa sao o ano inteiro!!!". Reescrito: inglês como língua franca (não específico do Reino Unido), e ponderação por densidade de Alojamento Local + sazonalidade em vez de estatísticas de uma única nacionalidade. Depois pediu explicitamente "muito mais páginas de en em porto e lisboa".

**Resultado final: 24 páginas em `/en/`** (`src/data/enTouristSeoData.ts` + `src/pages/EnServicePage.tsx` + `src/pages/EnGuidePage.tsx`):
- Porto: 8 páginas guest (Porto, Vila Nova de Gaia, Matosinhos, Póvoa de Varzim, Vila do Conde, Espinho, Braga, Guimarães) + 1 host = 9
- Lisboa: 8 páginas guest (Lisboa, Cascais, Sintra, Oeiras, Almada, Setúbal, Sesimbra, Mafra) + 1 host = 9
- Algarve: 4 páginas guest (Albufeira, Lagos, Portimão, Faro) + 1 host = 5
- 1 guia longo `/en/airbnb-portugal-cleaning-guide` (turnover, documentação para reclamações, preços reais), linkado pelas 3 páginas host.

**Isolamento total do PT** (confirmado, nada partilhado): `EnHeader.tsx`/`EnFooter.tsx` novos (sem menu PT, sem botão que abre o quiz PT); `ServiceFAQ.tsx` ganhou prop opcional `overline` (default PT inalterado) para poder reutilizar em inglês; `CookieBanner.tsx` e `MobileStickyBar.tsx` — que são globais em `App.tsx`, renderizados em TODAS as páginas — passaram a detetar `pathname.startsWith('/en/')` e mostrar texto em inglês só nesse caso (bug real encontrado ao testar: o banner de cookies e a barra fixa de WhatsApp mobile apareciam em português mesmo nas páginas em inglês). `scripts/prerender.ts` ganhou parâmetro `lang` no `emit()` para escrever `<html lang="en">` nessas rotas. `scripts/generate-sitemap.ts` ganhou `sitemap-en.xml` dedicado (24 URLs), separado dos sitemaps PT.

**Erro próprio corrigido a meio da sessão:** corri `generate-sitemap.ts` standalone para gerar o sitemap-en.xml em `public/` para o user analisar, mas isso recriou os 11 ficheiros `public/sitemap*.xml` que tinham sido removidos deliberadamente numa sessão anterior (WARNING fix: ficheiros estáticos que ficam desatualizados, o real é sempre gerado em `dist/` no build). Apaguei-os de novo e copiei só o `sitemap-en.xml` para a scratchpad, sem tocar em `public/`.

**Bugs reais encontrados e corrigidos ao vivo em `/problemas` (não pedidos, reportados pelo user a testar em paralelo):**
1. Texto dos cartões "O Problema"/"A Solução" cortado (`problemDetail`/`solutionDetail` têm 400-500 caracteres, a altura fixa do overlay sobre a foto nunca chegava). Corrigido movendo o texto para um painel sólido *abaixo* da foto (não sobreposto) — garante títulos sempre nivelados (mesma altura de foto nos dois lados) e nunca corta texto, seja qual for o tamanho.
2. Esse painel deixava um retângulo branco no fundo do cartão mais curto (grid esticava os cartões à mesma altura mas o painel não crescia). Corrigido com `flex-1` no painel.
3. Imagem "A Solução" era sempre a mesma foto em todas as páginas de sofá. Novo `getSolutionImage(serviceSlug, seed)` em `serviceGallery.ts` escolhe deterministicamente entre a foto "depois" + as fotos "Pormenor"/"Extração" da galeria.
4. `galeria-colchao-antes.webp`/`depois.webp` estavam trocados (o "antes" era o colchão limpo, o "depois" o sujo) — confirmado a olhar para os ficheiros, corrigido invertendo o mapeamento em `serviceGallery.ts` com comentário a explicar.
5. Secção "Como tratamos este problema" (timeline de 4 passos) redesenhada de círculos numerados ligados por linha para o mesmo grid de cartões premium usado no resto da página — aplicado a `ProblemPage.tsx` E `ProblemCityPage.tsx` (só o `ProblemPage` tinha sido tratado na sessão anterior). Primeira tentativa teve número duplicado (círculo pequeno + "01" ao lado, o user apontou a redundância); versão final usa o padrão já estabelecido nos cartões de "Benefícios" — um único número grande em marca-d'água a 8% de opacidade no canto.
6. `/limpeza-sofas` continuava a usar o par de fotos antigo apesar do par novo (`galeria-sofa-antes-2.webp`/`depois-2.webp`) já existir desde a sessão anterior — trocado.

**Padrão desta sessão:** o user testava tudo ao vivo com HMR ligado e ia mandando mensagens de correção sobrepostas ao que eu estava a fazer no momento (ex: pediu para corrigir o corte de texto em `/problemas` a meio da escrita dos dados EN) — geri isto tratando cada bug reportado imediatamente e voltando à tarefa principal depois, sempre confirmando por screenshot antes de assumir resolvido.

Commits: `84fee7d` (24 páginas EN base + fixes de `/problemas`), `e13ee42` (guia host EN). Ambos com push feito (`git remote` aponta para `github.com/tarzan49/sitekyroclean`), a pedido explícito do user antes de sair ("quando acabares... da commit push").

## Sessão — Investigação de indexação Google + audit completo ao site ao vivo (2026-08-10, continuação)

**Investigação de indexação (Google Search Console):** user reportou 1,17 mil páginas "Bloqueada pelo ficheiro robots.txt" + URLs `?lang=en`/`?lang=es` quebrados. Removida a regra `Disallow: /*?lang=*` do `robots.txt` (bloqueava a Google de voltar a rastrear URLs antigos do extinto sistema i18n, impedindo-a de perceber quais ainda existem vs quais já não existem). Adicionada tag `noindex, follow` a `NotFound.tsx` para dar um sinal claro nos casos que são mesmo 404.

**Bug real encontrado e corrigido — `/pack-sofa-colchao` e `/pack-sofa-impermeabilizacao`:** tinham HTML pré-renderizado real mas nenhuma rota React correspondente (o sistema de packs antigo, sem cidade, foi substituído por packs×cidade nalguma sessão anterior e estas 2 entradas do `prerender.ts` ficaram órfãs). Primeira passagem da Google via conteúdo real, passagem com JS mostrava sempre 404 — soft-404 clássico. Removidas do `prerender.ts`, substituídas por redirecionamento 301 em `public/_redirects` para `/pack-sofa-e-colchao-porto` e `/pack-sofa-impermeabilizacao-porto`.

**Bug real maior — os 27 posts do blog nunca tinham conteúdo pré-renderizado**, só a página de listagem `/blog`. Confirmado ao vivo: `curl` a qualquer `/blog/{slug}` devolvia o título e canonical da homepage, não do artigo. Isto é quase certamente a maior causa das 541 páginas "Página alternativa com etiqueta canónica correta" e parte das 468 "Rastreada, atualmente não indexada" — a Google via 27 URLs diferentes com o mesmo conteúdo (o da homepage) e tratava-os como duplicados. Corrigido com um loop novo em `prerender.ts` sobre `getAllPosts()`, espelhando o schema (BlogPosting, Breadcrumb, FAQ) já usado ao vivo em `BlogPost.tsx`.

**Método de investigação (repetível para o futuro):** em vez de adivinhar, escrito um script Node standalone (`audit-sitemap.js`, na scratchpad) que compara TODOS os URLs de todos os `sitemap-*.xml` contra os ficheiros `dist/{path}.html` realmente gerados — sinaliza (a) URLs sem ficheiro correspondente, (b) ficheiros cujo `<title>` bate certo com o da homepage (sinal de fallback/soft-404), (c) HTML com o texto da página 404, (d) HTML anormalmente pequeno. Foi assim que se encontraram os bugs do blog e dos packs órfãos. Depois da correção, o mesmo script confirmou **zero problemas em 9169 URLs**. Vale a pena reutilizar este script sempre que houver dúvidas de indexação no futuro.

**Nova regra de preço aplicada nesta sessão (fora do âmbito da indexação):** taxa de deslocação em Lisboa e Algarve passou a ter mínimo 10€, máximo 15€ (Porto/Norte inalterado, 5€-20€ como estava). Corrigido nos dois sítios onde a tabela existe (`QuizTypes.ts` e a cópia deliberada em `locationSeoData.ts`, usada pelo `prerender.ts` por não poder importar com alias `@/`), mais 2 respostas de FAQ que assumiam "a partir de 5€" em todas as regiões.

**Audit completo ao site ao vivo (a pedido explícito do user antes de sair de casa — "fica a trabalhar"):** verificado em produção (`cleansolutions.com.pt`, não localhost) e sem encontrar nenhum problema novo:
- Fluxo completo do quiz (localização Lisboa → +10€ correto, sofá 2 lugares 69€, total 79€ certo, sem desconto indevido — o "72€" visto a meio foi só o contador animado de preço a subir, não um bug; campos de nome/telefone do passo de contacto existem e aceitam texto sem erros de consola). Não submetido o formulário para não criar lead falso no CRM/WhatsApp reais.
- Amostra de 9 tipos de página diferentes (freguesia, keyword-variant, marca, problema, pack, preço, EN serviço, EN guia, redirecionamento legado) — todas sem erros de consola, todas com o título/preço correto.
- Viewport mobile (390px): homepage, página de problema redesenhada, cartões Problema/Solução (sem corte de texto), secção Processo redesenhada — tudo a renderizar bem.
- Network requests: zero 404s em qualquer asset real do site (JS/CSS/imagens); os únicos erros 503 são chamadas de tracking do Google Ads/Analytics (irrelevantes para a experiência do cliente).
